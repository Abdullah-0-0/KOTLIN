...
*cela va très vite de se tromper ainsi, mais vous vous retrouverez avec un fichier Java au milieu de vos fichiers Kotlin.*
...
Ajoutez dans `Calcul.kt` le code suivant :
 ...
 >
 > **ATTENTION :** il est possible qu'IntelliJ ajoute automatiquement `package org.example` au tout début des fichiers `kt` ; vous pouvez supprimer cette ligne ou il vous faudra faire attention à vos `mport ...`
 >    
...
Ajoutez une classe `Personne` ; attention à bien choisir `Class` dans l'assistant de création.
